# -*- coding: utf-8 -*-
"""
按照示例代码风格进一步统一后的敏感性分析画图脚本（v3）：
1. 默认保存 SVG / PDF / PNG（其中 SVG、PDF 为矢量图）
2. 四面封闭坐标轴，刻度线朝里
3. 图例框样式对齐参考代码：框内纯白、边框浅灰、外侧轻微阴影
4. 全部中文标签
5. reward 纵轴从 0 开始；泄露率与合法信噪比回到紧凑留白风格
6. 三联柱状图仅保留一个居中的横轴中文，并增加纵向顶部留白
"""
...
OUTPUT_ROOT = "./figures_sensitivity_handmake_38.25"
SAVE_SVG = True
SAVE_PDF = True
SAVE_PNG = True
...
# 3. 风格参数（对齐示例代码）
# ============================================================
LINE_COLORS = ["#E45756", "#4C78A8", "#54A24B"]
BAR_FACE_COLORS = ["#F2A6BE", "#84B6E3", "#8FCB81"]
BAR_EDGE_COLORS = ["#D97C9C", "#5B95CC", "#5FA653"]
...
# 4. 工具函数
# ============================================================
def apply_reference_legend(ax, legend_loc="best"):
    leg = ax.legend(
        loc=legend_loc,
        frameon=True,
        framealpha=0.90,
...
# def apply_reference_legend(ax, loc="best"):
#     leg = ax.legend(
#         loc=loc,
#         frameon=True,
#         fancybox=False,
...
def set_compact_ylim(ax, values, bottom=None, pad_ratio=0.10, min_span=None):
    ...
    lower = data_min if bottom is None else float(bottom)
    upper = data_max
    ...
    span = upper - lower
    if span == 0:
        span = abs(upper) * 0.1 if upper != 0 else 1.0
    ...
    top_pad = span * pad_ratio
    low_pad = 0.0 if bottom is not None else span * pad_ratio * 0.35
    ax.set_ylim(lower - low_pad, upper + top_pad)
...
def plot_one_experiment(exp_name, exp_cfg, exp_data):
    ...
    for label in labels_sorted:
        ...
        y_s = smooth_series(y, method=SMOOTH_METHOD, rolling_window=ROLLING_WINDOW, ema_alpha=EMA_ALPHA)
        leakage_all.extend(y_s[np.isfinite(y_s)])
        ax.plot(x, y_s, color=color, linewidth=MAIN_LINEWIDTH, label=f"{legend_prefix}={label}")

    ax.set_xlabel("训练步数（×10^6）")
    ax.set_ylabel("评估泄露率")
    ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0, decimals=0))
    ax.set_ylim(*ylims["leakage"])
    ax.set_xlim(0, 1)
    ax.set_xticks(np.linspace(0, 1, 6))
    ax.grid(True, linestyle="--", alpha=GRID_ALPHA_LINE)
    setup_closed_axes(ax)
    ...
    for label in labels_sorted:
        ...
        y_s = smooth_series(y, method=SMOOTH_METHOD, rolling_window=ROLLING_WINDOW, ema_alpha=EMA_ALPHA)
        legal_snr_all.extend(y_s[np.isfinite(y_s)])
        ax.plot(x, y_s, color=color, linewidth=MAIN_LINEWIDTH, label=f"{legend_prefix}={label}")

    ax.set_xlabel("训练步数（×10^6）")
    ax.set_ylabel("评估合法信噪比（dB）")
    ax.set_ylim(*ylims["legal_snr"])
    ax.set_xlim(0, 1)
    ax.set_xticks(np.linspace(0, 1, 6))
    ax.grid(True, linestyle="--", alpha=GRID_ALPHA_LINE)
    setup_closed_axes(ax)
    ...
    for ax, (metric_key, values) in zip(axes, metric_data):
        ...
        ax.set_xticks(x)
        xtick_labels = [str(labels_sorted[0]), f"{labels_sorted[1]}\n{x_label}", str(labels_sorted[2])]
        ax.set_xticklabels(xtick_labels)
        ax.set_xlabel("")
        ax.set_ylabel("")
        ax.set_title(FINAL_SUBPLOT_TITLES[metric_key], fontsize=19, pad=8)

        if exp_name == "eav_penalty_coef" and metric_key == "final_eval_reward":
            ax.set_ylim(37, 38.25)
        elif exp_name == "E_tot" and metric_key == "final_legal_snr_db":
            ax.set_ylim(5, 12)
        elif metric_key == "final_leakage_rate":
            ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0, decimals=0))
            set_compact_ylim(ax, values, bottom=0, pad_ratio=0.16)
        else:
            set_compact_ylim(ax, values, bottom=0, pad_ratio=0.16)
