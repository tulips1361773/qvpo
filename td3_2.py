...
class TD3Agent:
    ...
    def get_exploration_noise(self, step, start_steps):
        ...
        if step <= anneal_start: return self.exploration_noise_start
        if step >= self.exploration_noise_anneal_end: return self.exploration_noise_min
    ...
    def load_model(self, dir_path, id=None):
        ...
        self.actor_target = copy.deepcopy(self.actor); self.critic_target = copy.deepcopy(self.critic)

# ============================================================
# Evaluation & Args (完整保留所有参数定义)
# ============================================================

def evaluate(env, agent, steps, episodes=30):
    returns = np.zeros((episodes,), dtype=np.float32)
    eval_leakage_count, eval_total_users = 0, 0
    legal_snr_db_list, eav_snr_max_db_list = [], []

    for i in range(episodes):
        state, _ = env.reset()
        episode_reward = 0.0
        done = truncated = False
        ep_legal_snr_list, ep_eav_snr_max_list = [], []
        ...
        while not (done or truncated):
            ...
            if not np.isnan(legal_snr): ep_legal_snr_list.append(legal_snr)
            eav_snr_list = info.get('eavesdropper_snr_list', [])
            if len(eav_snr_list) > 0: ep_eav_snr_max_list.append(max(eav_snr_list))

        returns[i] = episode_reward
        if len(ep_legal_snr_list) > 0: legal_snr_db_list.append(np.mean(ep_legal_snr_list))
        if len(ep_eav_snr_max_list) > 0: eav_snr_max_db_list.append(np.mean(ep_eav_snr_max_list))
    ...
    print('-' * 60)
    print(f'Steps: {steps:<7}  reward: {mean_return:<7.2f}  leakage_rate: {eval_leakage_rate:.2%}')
    print('-' * 60)
    ...
    return {
        ...
        'snr_gap_db_mean': float(legal_snr_db_mean - eav_snr_max_db_mean) if not np.isnan(legal_snr_db_mean) else 0.0,
...
def readParser():
    ...
    parser.add_argument('--tau', type=float, default=0.005) # 默认值，会被 Bash 覆盖 [cite: 28]
...
def main(args=None, id=None):
    ...
    csv_logger = CSVExperimentLogger(run_id=id or '0', algorithm='TD3', seed=args.seed, 
                                     scenario_name=create_scenario_name(args), eval_interval=10000, csv_dir=os.path.join(log_dir, 'csv_logs'))
    ...
    if args.load_id: agent.load_model('./results/td3_Env', id=args.load_id)

    steps, episodes = 0, 0
    best_result, last_ep_reward = -float('inf'), np.nan
    recent_rewards = deque(maxlen=100)

    while steps < args.num_steps:
        state, _ = env.reset(seed=args.seed + episodes)
        episodes += 1; ep_reward, ep_steps = 0.0, 0
        done = truncated = False
        
        while not (done or truncated):
            if steps < args.start_steps: action = env.action_space.sample()
            else: action = agent.sample_action(state, step=steps, start_steps=args.start_steps)
            
            next_state, reward, done, truncated, info = env.step(action)
            memory.append(state, action, reward, next_state, 0.0 if (done or truncated) else 1.0)
            
            steps += 1; ep_steps += 1; ep_reward += reward
            if steps >= args.start_steps: agent.train(memory, args.batch_size, writer, steps)
            
            if steps % 200 == 0:
                writer.add_scalar('reward_terms/reward_final', float(reward), steps)
                if steps >= args.start_steps:
                    writer.add_scalar('policy/exploration_noise_std', agent.get_exploration_noise(steps, args.start_steps), steps)

            if steps % 10000 == 0:
                eval_res = evaluate(eval_env, agent, steps)
                writer.add_scalar('reward/eval_mean', eval_res['mean_return'], steps)
                csv_logger.log_training_metrics(eval_results=eval_res, step=steps, time_elapsed_sec=0, 
                                                train_reward=last_ep_reward, train_reward_ma100=np.mean(recent_rewards) if recent_rewards else 0)
            ...
            state = next_state
            if steps >= args.num_steps: break
        
        last_ep_reward = ep_reward; recent_rewards.append(ep_reward)
